...
sqlUpdateParams += ` iuo_slika = :iuo_slika, `;
}
if (arrayHas(allParams, 'iuo_slika_unutra')) {
sqlUpdateParams += ` iuo_slika_unutra = :iuo_slika_unutra, `;
}
...
if (arrayHas(allParams, 'pr_id')) {
sqlUpdateParams += ` pr_id = :pr_id `;
}

let updateSql = `update pr_info_obj  set ${sqlUpdateParams}  where pr_id = :pr_id`;
